...
const reportQuerySchema = z.object({
  ...
  groupBy: z.enum(['day', 'week', 'month', 'event']).optional().default('event'),
});

// GET /organizer/report - Returns comprehensive sales statistics for the organizer
router.get('/report', requireAuth(['ORGANIZER']), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const ticketTypeDetails = await prisma.ticketType.findMany({
      where: {
        id: {
...
router.get('/events', requireAuth(['ORGANIZER']), async (req: AuthenticatedRequest, res) => {
  try {
